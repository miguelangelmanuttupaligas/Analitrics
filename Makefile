	...
	case "$@" in \
		up) if [[ "$(STACK)" == "phoenix" ]]; then \
			...
			if [[ "$(STACK)" == "librechat" ]]; then \
				...
				docker compose --project-name "$$compose_project" --project-directory "$$compose_dir" -f "$$compose_dir/docker-compose.yml" up -d --wait --wait-timeout 150 --no-build control-postgres; \
				docker compose --project-name "$$compose_project" --project-directory "$$compose_dir" -f "$$compose_dir/docker-compose.yml" stop analytics-agent >/dev/null 2>&1 || true; \
				$(MAKE) --no-print-directory control-plane-migrate; \
			...
			if [[ "$(STACK)" == "librechat" ]]; then \
				...
				docker exec analitrics-analitrics-mongodb mongosh LibreChat --quiet --eval 'const tenantId="analitrics"; const now=new Date(); const missingTenant={$$or:[{tenantId:{$$exists:false}},{tenantId:null},{tenantId:""}]}; const users=db.users.updateMany(missingTenant,{$$set:{tenantId,updatedAt:now}}); const files=db.files.updateMany({source:"s3",$$or:[{tenantId:{$$exists:false}},{tenantId:null},{tenantId:""}]},{$$set:{tenantId,updatedAt:now}}); printjson({ok:true,tenantId,users:{matched:users.matchedCount,modified:users.modifiedCount},files:{matched:files.matchedCount,modified:files.modifiedCount}});'; \
				docker exec analitrics-analitrics-mongodb mongosh LibreChat --quiet --eval 'const locked={PROMPTS:{USE:false,CREATE:false,SHARE:false,SHARE_PUBLIC:false},AGENTS:{USE:true,CREATE:false,SHARE:false,SHARE_PUBLIC:false},RUN_CODE:{USE:false},WEB_SEARCH:{USE:false},MARKETPLACE:{USE:false},MCP_SERVERS:{USE:false,CREATE:false,SHARE:false,SHARE_PUBLIC:false,CONFIGURE_OBO:false},REMOTE_AGENTS:{USE:false,CREATE:false,SHARE:false,SHARE_PUBLIC:false},SKILLS:{USE:false,CREATE:false,SHARE:false,SHARE_PUBLIC:false}}; const res=db.roles.updateMany({name:{$$in:["USER","ADMIN"]}},{$$set:Object.fromEntries(Object.entries(locked).map(([k,v])=>["permissions."+k,v]))}); printjson({matched:res.matchedCount,modified:res.modifiedCount,locked:Object.keys(locked)});'; \
				docker exec analitrics-analitrics-mongodb mongosh LibreChat --quiet --eval 'const systemEmail="analitrics-system@internal.invalid"; const now=new Date(); db.users.updateOne({email:systemEmail},{$$set:{tenantId:"analitrics",updatedAt:now},$$setOnInsert:{name:"Analitrics System",username:"analitrics-system",email:systemEmail,emailVerified:false,provider:"system",idOnTheSource:"analitrics-system",role:"USER",createdAt:now}},{upsert:true}); const author=db.users.findOne({email:systemEmail},{_id:1,email:1,name:1,provider:1,tenantId:1,password:1,openidId:1,openidIssuer:1}); if(!author){throw new Error("system agent author was not created");} if(author.password||author.openidId||author.openidIssuer||author.provider!=="system"||author.tenantId!=="analitrics"){throw new Error("system agent author has an invalid authentication or tenant profile");} const viewer=db.accessroles.findOne({accessRoleId:"agent_viewer",resourceType:"agent"},{_id:1,permBits:1}); const owner=db.accessroles.findOne({accessRoleId:"agent_owner",resourceType:"agent"},{_id:1,permBits:1}); if(!viewer||!owner){throw new Error("agent access roles not found");} const existing=db.agents.findOne({id:"agent_analitrics",tenantId:"analitrics"},{_id:1}); const agentId=existing?existing._id:new ObjectId(); db.agents.updateOne({id:"agent_analitrics",tenantId:"analitrics"},{$$set:{_id:agentId,id:"agent_analitrics",name:"Analitrics",description:"Agente analítico único de Analitrics.",instructions:"Responde únicamente preguntas analíticas sobre los archivos cargados por el usuario.",provider:"openAI",model:"analitrics-agent",model_parameters:{},tools:[],tool_resources:{},author:author._id,authorName:"Analitrics System",category:"general",execute_code:false,file_search:false,web_search:false,is_promoted:true,tenantId:"analitrics",updatedAt:now},$$setOnInsert:{createdAt:now}},{upsert:true}); db.aclentries.updateOne({principalType:"public",resourceType:"agent",resourceId:agentId},{$$set:{tenantId:"analitrics",permBits:viewer.permBits,roleId:viewer._id,grantedBy:author._id,grantedAt:now}},{upsert:true}); db.aclentries.updateOne({principalType:"user",principalId:author._id,principalModel:"User",resourceType:"agent",resourceId:agentId},{$$set:{tenantId:"analitrics",permBits:owner.permBits,roleId:owner._id,grantedBy:author._id,grantedAt:now}},{upsert:true}); printjson({ok:true,id:"agent_analitrics",_id:agentId,author:{email:author.email,provider:author.provider,tenantId:author.tenantId},publicPermBits:viewer.permBits,ownerPermBits:owner.permBits});'; \
			fi ;; \
