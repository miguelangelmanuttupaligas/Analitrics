...
const DragDropModal = ({ onOptionSelect, setShowModal, files, isVisible }: DragDropModalProps) => {
  ...
  const options = useMemo(() => {
    ...
    const isTabularFile = (file: File) => {
      const type = getFileType(file);
      return type != null && tabularMimeTypes.has(type);
    };

    if (capabilities.contextEnabled) {
      _options.push({
        label: localize('com_ui_upload_tabular_analytics'),
        value: undefined,
        icon: <FileSpreadsheet className="icon-md" />,
        condition: files.length > 0 && files.every(isTabularFile),
      });
    }
